# SF-BMA: Bayesian model averaging and selection for stochastic frontier models 
...
Because exhaustive search evaluates all possible subsets of candidate regressors (i.e., potential expalanatory variables), its computational cost increases exponentially with the number of regressors. Also, in SFA some variable choices are driven by theory rather than by pure statistical fit. That is why the algorithm (as well as most of others below) allows us to provide an initial set of variables, which are not subjected to BMA/S (i.e., PIP=1 by default). Just place their columns as the first ones from the left right after the constant column (which should be, by default, the first column from the left in X). Parameter `init_var` determines how many columns in X, counting from left, are to be left out from the search (treated as fixed, or as the baseline model). The minimum and in many cases probably the default value is `init_var=1`, which means that only the intercept is left out from BMA (so the baseline model is an empty one).  
