function output = full_bs(tabela, y, n, T, sfa, init_var, dec_cr)
%FULL_BS Full backward search path for SFA models.
%
% The algorithm starts from the full model containing X0 and all candidate
% regressors. At each step it removes the candidate regressor with the
% lowest absolute t-ratio in the currently fitted model. There is no
% stopping criterion: the path continues until only X0 remains.
%
...
for step = 1:max_models
    ...
    % Stop after storing the X0-only model.
    if isempty(current_vars)
        break
    end

    % Backward rule: remove variable with the lowest absolute t-ratio.
    % This uses the fitted current model; no criterion stopping rule is used.
    % This would be an equivalent to mdd criterion
    beta_pos_model = init_var + (1:numel(current_vars));
    t_abs = abs(theta(beta_pos_model) ./ se(beta_pos_model));

    % If numerical issues occur, remove that variable first.
...
var_between = ((all_par_for_var - BMA_par').^2)' * p_prob_mdl;
