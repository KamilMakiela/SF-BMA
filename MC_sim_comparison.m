...
%for a = 19:27
%    id_SFA = mc_res{b,1}.sum1{a,6}(:,1);
%    mdd_SFA = mc_res{b,1}.sum1{a,6}(:,2);
%    BIC_SFA = mc_res{b,1}.sum1{a,6}(:,3);
%    id_classic = mc_res{b,1}.sum0{a,6}(:,1);
%    BIC_classic = mc_res{b,1}.sum0{a,6}(:,3);

%    ranking = compare_model_rankings(id_SFA, id_classic, mdd_SFA, BIC_SFA, BIC_classic, 1000);
%    rank_summary = [rank_summary; ranking.summary];
%end
...
for j = 1:numel(metric_vars)
    ...
    rank_mean.(v) = splitapply(@(x) mean(x, 'omitnan'), ...
        rank_summary.(v), G);

    rank_std.(v) = splitapply(@(x) std(x, 'omitnan'), ...
        rank_summary.(v), G);
